  ...
  <div class="justify-center bg-white mt-5 ml-[275px] mr-[20px] rounded-xl shadow-lg flex-col">
    <div>
      <div class="border border-black rounded-lg  w-full">
        <form method="GET" action="<?= base_url('/admin/history/document/' . $Id) ?>" class="py-3 px-4 rounded-lg">
            ...
            <tbody class="text-md divide-y divide-black">
              <?php foreach ($docshistory as $docs): ?>
                <tr>
                  ...
                  <td class="p-3">
                    <a href="<?= base_url('/admin/download/' . $docs['Id']) ?>" class="inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent text-blue-600
